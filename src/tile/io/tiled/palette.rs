...
use super::import::{TiledMapSnapshot, TiledTileset};
use super::properties::property_bag_from_tiled;

pub(super) fn import_palettes(import: &TiledMapSnapshot) -> Vec<TilePalette> {
    import
        .tilesets
        .iter()
...
fn tile_palette_from_tileset(tileset: &TiledTileset) -> TilePalette {
    ...
    for id in 0..tileset.tile_count {
        let rect = tileset_rect(tileset, id);
        let mut tile = TileDef::new(TileDefId(id), rect);
        tile.draw_size = tileset.tile_draw_size(id);
        tile.draw_offset = tileset.tile_offset;
        tile.animation = tileset.animation(TileDefId(id));
...
fn tileset_rect(tileset: &TiledTileset, tile_id: u32) -> RectU {
    if let Some(Some(rect)) = tileset.tile_rects.get(tile_id as usize) {
        return *rect;
    }
