...
use super::super::object::{TileObject, TileObjectId};
use super::super::palette::PropertyBag;
use super::data::{LayerData, ObjectLayerData, TileLayerData};
use super::tile::SceneTile;
...
impl TileLayer {
    ...
    pub fn set_tile(&mut self, cell: CellCoord, tile: Option<SceneTile>) -> Option<SceneTile> {
        self.data.as_tiles_mut()?.tiles.set(cell, tile)
    }

    pub fn tile(&self, cell: CellCoord) -> Option<SceneTile> {
        self.data.as_tiles()?.tiles.get(cell)
    }

    pub fn add_object(&mut self, object: &TileObject) {
        self.add_object_id(object.id);
    }
