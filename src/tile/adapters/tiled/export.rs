...
use crate::tile::{
    GridOrientation, LayerData, LayerId, ObjectVisual, PaletteId, PropertyBag, PropertyValue,
    TileDef, TileDefId, TileLayer, TileMap, TileMapDocument, TileObject, TilePalette,
    TilePaletteStore, TileTextureSource,
};
...
impl TiledExporter {
    ...
    pub fn document_to_tmj_string(document: &TileMapDocument) -> Result<String, TiledExportError> {
        let palette_order = scene_palette_order_from_store(&document.scene, &document.palettes)?;
        let map = export_tmj_value(&document.scene, &palette_order)?;
        serde_json::to_string_pretty(&map).map_err(TiledExportError::Json)
    }
    ...
    pub fn write_document_tmj(
        path: impl AsRef<Path>,
        document: &TileMapDocument,
    ) -> Result<(), TiledExportError> {
        let path = path.as_ref();
        let text = Self::document_to_tmj_string(document)?;
        std::fs::write(path, text).map_err(|source| TiledExportError::Io {
            path: path.to_path_buf(),
            source,
        })
...
fn scene_palette_order_from_store<'a>(
    scene: &TileMap,
    palettes: &'a TilePaletteStore,
) -> Result<Vec<&'a TilePalette>, TiledExportError> {
    let mut ordered = Vec::new();
    for id in &scene.palettes {
        ordered.push(
...
fn common_layer_fields(layer: &TileLayer, layer_type: &'static str) -> Map<String, Value> {
...
fn render_order_name(order: crate::render::TilemapRenderOrder) -> &'static str {
    match order {
        crate::render::TilemapRenderOrder::RightDown => "right-down",
        crate::render::TilemapRenderOrder::RightUp => "right-up",
        crate::render::TilemapRenderOrder::LeftDown => "left-down",
        crate::render::TilemapRenderOrder::LeftUp => "left-up",
    }
}
