...
fn built_in_phase_execute(
    ...
    while cursor < phase_items.len() {
        ...
        let mut draw_result = Ok(());
        {
            ...
            let scene_shadows = execution
                .view_state()
                .scene_shadows()
                .cloned()
                .or_else(|| execution.view_payload::<SceneShadowResources>().cloned());
            let indirect_lighting = execution.frame_payload::<FrameIndirectLighting>();
            let mut draw_ctx = DrawContext::new(
                ...
                indirect_lighting
                    .cloned()
                    .map(|resources| resources.sampling().clone()),
                indirect_lighting.map(|resources| resources.shader().source),
                indirect_lighting.map_or(0, |resources| gi_shader_key(resources.shader().key)),
                material_registry,
                mesh_registry,
                Some(fallback_texture),
                target.format(),
