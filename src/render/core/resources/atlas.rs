...
impl std::fmt::Display for AtlasError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::DuplicateName { name } => write!(f, "Duplicate atlas entry \"{name}\""),
            Self::PixelDataSizeMismatch { expected, actual } => write!(
                f,
                "Atlas entry pixel data size mismatch: expected {expected} bytes, got {actual}"
            ),
            Self::EntriesDoNotFit => write!(f, "Atlas entries do not fit in the requested atlas"),
        }
    }
}
...
impl AtlasPacker {
    ...
    pub fn add(
        ...
        if pixels.len() != expected_len {
            return Err(AtlasError::PixelDataSizeMismatch {
    ...
    ///
    /// Returns an error if the entries don't fit in the atlas dimensions.
    pub fn build(mut self, ctx: &GpuContext) -> Result<TextureAtlas, AtlasError> {
        ...
        for entry in &self.entries {
            let rect = packer
                .pack(entry.width, entry.height)
                .ok_or(AtlasError::EntriesDoNotFit)?;
        ...
        // Create GPU texture.
        let texture = Texture::from_rgba8_with_label(ctx, atlas_w, atlas_h, &atlas_pixels, "atlas");
...
mod tests {
    ...
    fn duplicate_names_are_rejected() {
        ...
        packer.add("hero", 4, 4, &[255; 4 * 4 * 4]).unwrap();
        let err = match packer.add("hero", 4, 4, &[0; 4 * 4 * 4]) {
            Ok(_) => panic!("duplicate atlas entry should fail"),
            Err(err) => err,
        };
        assert_eq!(
            err,
    ...
    #[test]
    fn bad_pixel_lengths_are_rejected() {
        let mut packer = AtlasPacker::new(32, 32);
        let err = match packer.add("hero", 4, 4, &[255; 8]) {
            Ok(_) => panic!("bad atlas pixel length should fail"),
            Err(err) => err,
        };
        assert_eq!(
            err,
            AtlasError::PixelDataSizeMismatch {
                expected: 4 * 4 * 4,
                actual: 8,
            }
        );
