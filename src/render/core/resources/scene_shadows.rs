//! Renderer-family-neutral scene shadow binding contracts.

use crate::render::gpu::RenderTarget;
use crate::render::graph::ImportedTexture;
use crate::render::graph::TextureHandle;

/// Maximum directional-shadow cascades supported by the shared scene contract.
...
/// Imported directional-shadow atlas metadata consumed by debug visualization.
///
/// This is neutral frame data: the lighting feature creates it and the post-fx
/// debug feature reads it without either family depending on the other.
#[derive(Clone, Debug)]
...
impl ShadowDebugResources {
    ...
    #[inline]
    pub(crate) fn from_directional_atlas(
        target: &RenderTarget,
        cascade_count: u32,
        directional_shadow_mul_add: [f32; 4],
    ) -> Self {
...
mod tests {
    ...
    #[test]
    fn directional_shadow_debug_resource_keeps_atlas_slice_metadata() {
