...
mod tests {
    ...
    #[test]
    fn descriptors_clamp_zero_counts_and_resize_preserves_array_layers() {
        let (device, queue) = create_test_device();
        let ctx =
            GpuContext::new_headless(device, queue, wgpu::TextureFormat::Rgba8Unorm, [32, 32]);
        let mut target = RenderTarget::from_descriptor(
            ...
            RenderTargetDescriptor {
                ...
                mip_level_count: 0,
                array_layer_count: 3,
                label: "array_target".into(),
            },
        );

        assert_eq!(target.sample_count(), 1);
        assert_eq!(target.mip_level_count(), 1);
        assert_eq!(target.array_layer_count(), 3);

        target.resize(&ctx, 16, 12, wgpu::TextureFormat::Rgba8Unorm);
