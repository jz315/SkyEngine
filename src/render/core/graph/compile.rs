...
impl RenderGraph {
    ...
    pub fn compile(&mut self) -> Result<Vec<CompiledPass>, RenderGraphError> {
        ...
        Ok(self.cached_compiled.clone())
    }

    /// Compile and cache the graph without cloning the compiled pass list.
    ///
    /// Frame execution uses this path because it consumes the cached list
    /// directly. The public [`Self::compile`] API still returns an owned
    /// snapshot for callers that need to inspect it.
    pub(crate) fn ensure_compiled(&mut self) -> Result<(), RenderGraphError> {
        if self.compiled {
            return Ok(());
        }
        ...
        self.cached_compiled = compiled_passes;
        self.compiled = true;
        Ok(())
    }
    ...
    fn analyze_dependencies(&self) -> Result<DependencyAnalysisResult, RenderGraphError> {
        ...
        for (idx, pass) in self.passes.iter().enumerate() {
            ...
            for &resource in access.reads {
                ...
                if !has_writer && !self.resource_has_external_source(resource) {
