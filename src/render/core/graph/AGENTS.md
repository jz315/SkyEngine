# AGENTS.md — `src/render/core/graph`
...
- `ResourceRef` is the unified reference type: `Surface`, `Texture(TextureHandle)`, `TextureSubresource(TextureSubresource)`, or `Buffer(BufferHandle)`.
...
1. **Dependency analysis**: scan all passes in declaration order. For each read, edge from overlapping last writers → this pass. For each write, edge from overlapping last writers and prior readers → this pass. Detect `ReadBeforeWrite` for resources that are neither externally provided nor persistent across frames. Whole-texture reads/writes overlap every subresource of that texture; subresource conflicts are limited to overlapping mip/layer ranges. Uses Kahn's algorithm with index-sorted ready queue for deterministic topological order. Detects cycles.
...
- **Transient textures**: allocated from `TransientPool` on first use, returned after frame execution via `release_transient_resources()`. Pool key is `{format, usage, width, height, sample_count, mip_level_count, array_layer_count}`.
- Virtual texture descriptors also track `usage`, `sample_count`, `mip_level_count`, and `array_layer_count`; pooling and aliasing require those values to match exactly.
...
- **Physical subresource views**: `PhysicalResources` can create per-mip/per-layer sampled/storage/render-attachment views.  Storage views assert `STORAGE_BINDING`; render-attachment views assert exactly one mip and one array layer.
...
- `runtime/pipeline_runtime.rs` is the high-level bridge that converts `PipelineStep`s into `FramePipeline` nodes backed by `RenderGraph`.
- Physical textures are backed by `RenderTarget` (`src/render/gpu/target.rs`).
- Higher-level passes such as `lighting::LightPass`, `composite::CompositePass`, and post-fx nodes build on top of `RenderGraph` by calling `add_render_pass`/`add_compute_pass` and resolving physical resources from `PhysicalResources`.
- The `Blackboard` shared data system lives at `src/render/resources/blackboard.rs`.
