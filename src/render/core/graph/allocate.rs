...
#[derive(Default)]
pub(super) struct AllocationScratch {
    effective_texture_usages: Vec<wgpu::TextureUsages>,
    forbidden_alias_pairs: FxHashSet<(usize, usize)>,
    pass_texture_indices: Vec<usize>,
    aliased_texture_indices: Vec<bool>,
}

impl RenderGraph {
    // ── Physical resource management ────────────────────────────────────

    fn collect_pass_local_texture_alias_conflicts(&self, scratch: &mut AllocationScratch) {
        scratch.forbidden_alias_pairs.clear();
        for &pass_idx in &self.order {
            let pass = &self.passes[pass_idx];
            scratch.pass_texture_indices.clear();
            for resource in pass.reads.iter().chain(pass.writes.iter()).copied() {
                match resource {
                    ResourceRef::Texture(handle) => scratch.pass_texture_indices.push(handle.0),
                    ResourceRef::TextureSubresource(subresource) => {
                        scratch.pass_texture_indices.push(subresource.texture.0);
                    }
                    ResourceRef::Surface | ResourceRef::Buffer(_) => {}
                }
            }

            scratch.pass_texture_indices.sort_unstable();
            scratch.pass_texture_indices.dedup();
            for left in 0..scratch.pass_texture_indices.len() {
                for right in left + 1..scratch.pass_texture_indices.len() {
                    scratch.forbidden_alias_pairs.insert((
                        scratch.pass_texture_indices[left],
                        scratch.pass_texture_indices[right],
                    ));
                }
            }
        }
    ...
    pub fn allocate_physical_resources(&mut self, ctx: &GpuContext) {
        ...
        let mut scratch = std::mem::take(&mut self.allocation_scratch);
        self.physical_textures
            .resize_with(self.textures.len(), || None);
        self.physical_buffers
            .resize_with(self.buffers.len(), || None);
        scratch.effective_texture_usages.clear();
        scratch
            .effective_texture_usages
            .reserve(self.textures.len());
        for tex_idx in 0..self.textures.len() {
            scratch
                .effective_texture_usages
                .push(self.texture_usage_for(TextureHandle(tex_idx, self.handle_token)));
        }
        ...
        for group in &self.alias_groups {
            ...
            let first_name = self.textures[group.members[0]].name.as_ref();
            let additional_members = group.members.len() - 1;
            let shared_target = self.transient_pool.acquire_with_label(ctx, key, || {
                Cow::Owned(format!("alias_group[{first_name}+{additional_members}]"))
            });
        ...
        for (tex_idx, &effective_usage) in scratch
            .effective_texture_usages
            .iter()
            .enumerate()
            .take(self.textures.len())
        {
            ...
            if scratch.aliased_texture_indices[tex_idx] {
                continue;
            }
        ...
        self.allocation_scratch = scratch;
    }
