...
#[test]
fn transient_pool_releases_descriptors_unused_after_resize_churn() {
    let (device, queue) = create_test_device();
    let ctx = crate::gpu::GpuContext::new_headless(
        device,
        queue,
    ...
    let target = pool.acquire(&ctx, key, "stale_resize_target".into());
    pool.release(key, target);
    assert!(pool.first_texture_ptr().is_some());

    pool.begin_frame();
    assert!(pool.first_texture_ptr().is_some());
    pool.begin_frame();
    assert!(pool.first_texture_ptr().is_none());
}
...
#[test]
fn clear_frame_drops_persistent_resources_unused_by_the_previous_frame() {
    let (device, queue) = create_test_device();
    let ctx = crate::gpu::GpuContext::new_headless(
        device,
        queue,
    ...
    let mut graph = RenderGraph::new();
    let texture = graph.create_texture(|builder| {
        builder
            .name("dynamic_persistent")
            .size(TargetSize::Exact(16, 16))
            .persistent();
    });
    graph.add_render_pass("write_dynamic_persistent", |setup| {
        setup.write(texture);
    });
    graph.compile().unwrap();
    graph.allocate_physical_resources(&ctx);
    ...
    graph.clear_frame();
    assert_eq!(graph.persistent_texture_cache.len(), 1);

    // No declaration claims the cached resource in this frame.
    graph.clear_frame();
    assert!(graph.persistent_texture_cache.is_empty());
}
