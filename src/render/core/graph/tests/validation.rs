...
#[test]
fn readwrite_transient_requires_an_earlier_writer() {
    let mut graph = RenderGraph::new();
    let texture = graph.create_texture(|b| {
        b.name("uninitialized_readwrite");
    });
    graph.add_compute_pass("readwrite_first", |setup| {
        setup.readwrite(texture);
        setup.write_surface();
    });
    ...
    assert!(matches!(
        ...
        }) if pass == "readwrite_first" && handle == texture
    ));
}

#[test]
fn loaded_attachment_transient_requires_an_earlier_writer() {
    let mut graph = RenderGraph::new();
    let texture = graph.create_texture(|b| {
        b.name("uninitialized_loaded_attachment");
    });
    graph.add_render_pass("load_first", |setup| {
        setup.write_color_loaded(0, texture);
        setup.write_surface();
    });
    ...
    assert!(matches!(
        ...
        Err(RenderGraphError::ReadBeforeWrite {
            pass,
            resource: ResourceRef::Texture(handle),
        }) if pass == "load_first" && handle == texture
    ));
}
