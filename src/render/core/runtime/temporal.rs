...
impl TemporalViewTracker {
    pub(crate) fn invalidate(&mut self) {
        ...
        self.temporal_aa_enabled = false;
    }
    ...
    pub(crate) fn update_views(
        ...
        for view in views {
            ...
            let previous = self
                .histories
                .get(&view.history_key())
                .copied()
                .filter(|history| history.last_seen_frame.wrapping_add(1) == frame_index);
        ...
        // A temporal chain is only valid across consecutive frames. Pruning
        // absent views also bounds memory when cameras are created and removed
        // over a long-running session.
        self.histories
            .retain(|_, history| history.last_seen_frame == frame_index);
...
mod tests {
    ...
    #[test]
    fn temporal_tracker_prunes_absent_views_and_resets_on_return() {
        let mut tracker = TemporalViewTracker::default();
        let mut first = vec![test_view(
            Transform::from_xyz(0.0, 0.0, 8.0),
            Projection::perspective(60.0f32.to_radians(), 0.1, 100.0),
        ...
        tracker.update_views(&mut [], true, 1.0);
        assert!(tracker.histories.is_empty());

        let mut returned = vec![test_view(
            Transform::from_xyz(2.0, 0.0, 8.0),
            Projection::perspective(60.0f32.to_radians(), 0.1, 100.0),
        )];
        tracker.update_views(&mut returned, true, 1.0);

        assert!(returned[0].temporal.history_reset);
        assert_eq!(
            returned[0].temporal.previous_view_proj,
            returned[0].temporal.current_view_proj
        );
    }

    #[test]
    fn temporal_tracker_resets_after_explicit_invalidation() {
        let mut tracker = TemporalViewTracker::default();
        let mut first = vec![test_view(
            Transform::from_xyz(0.0, 0.0, 8.0),
            Projection::perspective(60.0f32.to_radians(), 0.1, 100.0),
        ...
        let mut next = vec![test_view(
            Transform::from_xyz(1.0, 0.0, 8.0),
            Projection::perspective(60.0f32.to_radians(), 0.1, 100.0),
        )];
        tracker.update_views(&mut next, true, 1.0);

        assert!(next[0].temporal.history_reset);
        assert_eq!(
            next[0].temporal.previous_view_proj,
            next[0].temporal.current_view_proj
        );
    }
}
