...
struct FailingGraphPass;

impl GraphPass for FailingGraphPass {
    fn name(&self) -> &'static str {
        "failing_graph_pass"
    }
    ...
    fn setup(&mut self, ctx: &mut GraphPassSetupContext<'_, '_>) {
        let current = ctx
            .state()
            .current_color()
            .expect("scene color should exist before the failing test pass");
        ctx.graph().add_render_pass(self.name(), |setup| {
            setup.write_color_loaded(0, current.handle());
        });
    }
    ...
    fn execute(
        ...
        Err(RenderGraphError::ExecutionFailed(
...
#[test]
fn runtime_reports_render_graph_execution_failure_as_skipped() {
    let (device, queue) = create_test_device();
    let mut ctx =
        GpuContext::new_headless(device, queue, wgpu::TextureFormat::Bgra8Unorm, [16, 16]);
    let pipeline = RenderPipelineBuilder::new()
        .add_graph_pass(FailingGraphPass)
        .build();
    let mut renderer = RenderRuntime::from_asset(pipeline);
    let world = World::new();

    ctx.begin_frame()
        .expect("headless frame should begin for failure propagation test");
    let outcome = renderer.render_world(&mut ctx, &world);
    ctx.end_frame();

    assert_eq!(
        outcome,
        crate::render::FrameRenderOutcome::Skipped(
            crate::render::FrameSkipReason::RenderGraphFailed
        )
    );
    assert!(
        renderer.executor.cached_transient_texture_ptr().is_none(),
        "failed graphs must not carry partially written GPU resources into the next frame"
    );
}
