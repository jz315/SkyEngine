use super::common::*;
use crate::render::features::lighting::renderer::shadow::ShadowFrameExtension;

fn shadow_extension(renderer: &RenderRuntime) -> &ShadowFrameExtension {
    renderer
        .frame_extension::<ShadowFrameExtension>()
        .expect("forward 3D pipelines install the directional-shadow extension")
}
...
fn forward_3d_enables_shadow_view_for_perspective_directional_light() {
    ...
    let shadow_views = shadow_extension(&renderer).views();
    assert_eq!(shadow_views.len(), 1);
    assert!(shadow_views[0].enabled());
    assert!(shadow_views[0].caster_count() > 0);
    assert!((shadow_views[0].radius() - 0.05).abs() < 0.0001);
}
...
fn forward_3d_directional_shadow_atlas_writes_depth_for_casters() {
    ...
    let shadow_view = shadow_extension(&renderer)
        .views()
        .first()
...
fn directional_shadow_cascade_boundary_keeps_near_and_far_receivers_consistent() {
    ...
    let mut renderer = RenderRuntime::from_asset(
        RenderPipelineAsset::builder()
            .register_material::<StandardMaterial>()
            .add_phase(crate::render::lighting::shadow::DirectionalShadowPhase::new())
            .add_compute(crate::render::features::gi::GiUpdateCompute)
            .add_phase(crate::render::OpaquePhase::new())
