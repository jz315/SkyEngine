...
impl Camera2D {
    ...
    pub fn view_projection(&self) -> [f32; 16] {
        let hw = self.viewport_width * 0.5 / self.zoom;
        let hh = self.viewport_height * 0.5 / self.zoom;
    ...
    pub fn screen_to_world(&self, screen_x: f32, screen_y: f32) -> [f32; 2] {
        ...
        let world_x = self.position[0] + (screen_x / self.viewport_width - 0.5) * 2.0 * hw;
        // Flip Y: screen Y goes down, world Y goes up
        let world_y = self.position[1] - (screen_y / self.viewport_height - 0.5) * 2.0 * hh;
