...
## Role
- `src/render/integration/backend/` owns app-facing renderer selection and backend adapters.
...
## Kajiya Boundary
- SkyEngine's Kajiya adapter lives in `src/render/integration/backend/kajiya/`; vendored upstream code stays in `crates/vendor/kajiya/`.
- Kajiya types may only appear in `src/render/integration/backend/kajiya/` and vendored Kajiya code.
...
## Renderling Boundary
- SkyEngine's Renderling adapter lives in `src/render/integration/backend/renderling.rs`.
