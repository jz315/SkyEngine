...
use crate::render::view::{ResolvedSceneTransforms, SceneTransformResolver};
use crate::render::{
    CameraMarker, DirectionalLight, MeshRenderer, PointLight, RenderLayerMask, RenderSettings,
    SpotLight, Transform,
};
...
pub struct SceneSnapshotExtractor {
    ...
    seen_entities: FxHashSet<EntityId>,
...
impl SceneSnapshotExtractor {
    pub fn new() -> Self {
        Self {
            ...
            seen_entities: FxHashSet::default(),
    ...
    pub fn extract_into(&mut self, world: &World, snapshot: &mut SceneSnapshot) {
        self.transform_resolver
            .resolve_into(world, &mut self.resolved_transforms);
        let resolved_transforms = &self.resolved_transforms;
        ...
        let seen_entities = &mut self.seen_entities;
        self.mesh_query
            .for_each_with_entity(world, |entity, (transform, renderer, layer_mask)| {
                ...
                let transform = resolved_transforms.get(entity).unwrap_or(*transform);
                let effective_layer_mask = layer_mask.map_or(renderer.layer_mask, |mask| mask.0);
                let mut materials = snapshot
                    .mesh_instances
                    .get_mut(&entity)
                    .map(|instance| std::mem::take(&mut instance.materials))
                    .unwrap_or_default();
                materials.clear();
                materials.extend(renderer.materials.iter().map(|material| material.id()));
        ...
        snapshot
            .mesh_instances
            .retain(|entity, _| self.seen_entities.contains(entity));
