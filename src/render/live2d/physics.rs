    ...
    pub fn evaluate(&mut self, model: &mut Live2DModel, delta_time_seconds: f32) {
        ...
        while self.current_remain_time >= physics_delta_time {
            ...
            for index in 0..parameter_count {
                ...
                self.parameter_caches[index] =
                    self.parameter_input_caches[index] * (1.0 - input_weight)
            ...
            for (setting_index, rig) in self.sub_rigs.iter_mut().enumerate() {
                ...
                total_translation.x = total_translation.x * rad_angle.cos()
                    - total_translation.y * rad_angle.sin();
                total_translation.y = total_translation.x * rad_angle.sin()
                    + total_translation.y * rad_angle.cos();
...
fn parse_input(
    ...
    let source = input
        .get("Source")
        .ok_or_else(|| format!("physics setting {setting_index} input {input_index} missing Source"))?;
    let target = source
        .get("Target")
        .and_then(|v| v.as_str())
        .ok_or_else(|| format!("physics setting {setting_index} input {input_index} missing Source.Target"))?;
    if target != "Parameter" {
        return Err(format!(
    ...
    let id = source
        .get("Id")
        .and_then(|v| v.as_str())
        .ok_or_else(|| format!("physics setting {setting_index} input {input_index} missing Source.Id"))?;
    Ok(Input {
        parameter_index: model.find_parameter(id),
        weight: read_f32(input, "Weight", setting_index, input_index, "input")?,
        reflect: input.get("Reflect").and_then(|v| v.as_bool()).unwrap_or(false),
        source: parse_source(input.get("Type").and_then(|v| v.as_str()), setting_index, input_index, "input")?,
    })
}
...
fn parse_output(
    ...
    let target = destination
        .get("Target")
        .and_then(|v| v.as_str())
        .ok_or_else(|| format!("physics setting {setting_index} output {output_index} missing Destination.Target"))?;
    if target != "Parameter" {
        return Err(format!(
    ...
    let id = destination
        .get("Id")
        .and_then(|v| v.as_str())
        .ok_or_else(|| format!("physics setting {setting_index} output {output_index} missing Destination.Id"))?;
    ...
    Ok(Output {
        ...
        vertex_index: output
            .get("VertexIndex")
            .and_then(|v| v.as_u64())
            .ok_or_else(|| format!("physics setting {setting_index} output {output_index} missing VertexIndex"))?
            as usize,
        ...
        reflect: output.get("Reflect").and_then(|v| v.as_bool()).unwrap_or(false),
        source: parse_source(output.get("Type").and_then(|v| v.as_str()), setting_index, output_index, "output")?,
...
fn parse_particle(
    ...
    let position = parse_vec2(vertex.get("Position"), format!("physics setting {setting_index} vertex {vertex_index} position"))?;
    Ok(Particle {
        ...
        acceleration: read_f32(vertex, "Acceleration", setting_index, vertex_index, "vertex")?,
...
fn parse_normalization(
    ...
    Ok(Normalization {
        minimum: value.get("Minimum").and_then(|v| v.as_f64()).map(|v| v as f32).ok_or_else(|| format!("{} missing Minimum", label.as_ref()))?,
        maximum: value.get("Maximum").and_then(|v| v.as_f64()).map(|v| v as f32).ok_or_else(|| format!("{} missing Maximum", label.as_ref()))?,
        default: value.get("Default").and_then(|v| v.as_f64()).map(|v| v as f32).ok_or_else(|| format!("{} missing Default", label.as_ref()))?,
    })
}
...
fn parse_vec2(
    ...
    let value = value.ok_or_else(|| format!("{} missing", label.as_ref()))?;
    Ok(Vec2::new(
        value.get("X").and_then(|v| v.as_f64()).map(|v| v as f32).ok_or_else(|| format!("{} missing X", label.as_ref()))?,
        value.get("Y").and_then(|v| v.as_f64()).map(|v| v as f32).ok_or_else(|| format!("{} missing Y", label.as_ref()))?,
    ))
}
...
fn read_f32(
    ...
    value
        ...
        .and_then(|v| v.as_f64())
        .map(|v| v as f32)
        .ok_or_else(|| format!("physics setting {setting_index} {kind} {item_index} missing {field}"))
}
...
fn update_particles(
    ...
    for index in 1..strand.len() {
        ...
        let radian = direction_to_radian(strand[index].last_gravity, current_gravity) / AIR_RESISTANCE;
        ...
        strand[index].position += strand[index].velocity * delay + strand[index].force * delay * delay;
...
fn normalize_parameter_value(
    ...
    let value = value.clamp(parameter_minimum.min(parameter_maximum), parameter_minimum.max(parameter_maximum));
    ...
    let result = match sign(param_value) {
        1 => {
            ...
            if p_length != 0.0 { param_value * (n_length / p_length) + normalization.default } else { normalization.default }
        }
        -1 => {
