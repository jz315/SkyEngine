...
struct VertexOutput {
    ...
    @location(2) view_tangent: vec3<f32>,
    @location(3) view_bitangent: vec3<f32>,
};
...
fn vs_main(input: VertexInput) -> VertexOutput {
    ...
    let world_normal = safe_normalize((model * vec4<f32>(input.normal, 0.0)).xyz);
    let world_tangent_raw = (model * vec4<f32>(input.tangent.xyz, 0.0)).xyz;
    let world_tangent = safe_normalize(
        world_tangent_raw - world_normal * dot(world_normal, world_tangent_raw),
    );
    let world_bitangent = safe_normalize(cross(world_normal, world_tangent) * input.tangent.w);

    output.clip_position = camera.view_proj * world_position;
    output.uv = input.uv;
    output.view_normal = safe_normalize((camera.view * vec4<f32>(world_normal, 0.0)).xyz);
    output.view_tangent = safe_normalize((camera.view * vec4<f32>(world_tangent, 0.0)).xyz);
    output.view_bitangent = safe_normalize((camera.view * vec4<f32>(world_bitangent, 0.0)).xyz);
    return output;
}
...
fn fs_main(input: VertexOutput) -> FragmentOutput {
    ...
    let view_normal = safe_normalize(
        sampled.x * input.view_tangent
        + sampled.y * input.view_bitangent
        + sampled.z * input.view_normal
    );
    ...
    output.encoded_normal = vec4<f32>(view_normal * 0.5 + vec3<f32>(0.5), 1.0);
    return output;
}
