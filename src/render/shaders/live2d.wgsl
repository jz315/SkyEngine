...
@group(1) @binding(3) var destination_texture: texture_2d<f32>;
...
fn vs_main(input: VertexInput) -> VertexOutput {
    ...
    let ndc = output.sv_position.xy / output.sv_position.w;
    output.blend_uv = vec2<f32>(ndc.x * 0.5 + 0.5, 1.0 - (ndc.y * 0.5 + 0.5));
    return output;
}
...
fn sample_source_color(input: VertexOutput) -> vec4<f32> {
...
fn model_fs(input: VertexOutput) -> @location(0) vec4<f32> {
    return sample_source_color(input) * sample_mask(input);
}

@fragment
fn model_overlap_fs(input: VertexOutput) -> @location(0) vec4<f32> {
    let mask_value = sample_mask(input);
    let color_source = convert_premultiplied_to_straight(sample_source_color(input));
    let masked_source = vec4<f32>(color_source.rgb, color_source.a * mask_value);
    let color_destination =
        convert_premultiplied_to_straight(textureSample(destination_texture, tex_sampler, input.blend_uv));
    return alpha_blend(
