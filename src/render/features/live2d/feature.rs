...
use crate::render::phase::{
    transparent_sort_key, DrawFunctionId, Live2DDrawData, OpaquePhase, PhaseItem, TransparentPhase,
};
#[cfg(feature = "live2d")]
...
#[cfg(feature = "live2d")]
use super::{Live2DAnimator, Live2DCommand, Live2DCommands, Live2DModelInstance};
...
impl RenderFeature for Live2DFeature {
    ...
    fn register(&mut self, builder: &mut RenderPipelineBuilder) {
        self.set_draw_function_id(builder.register_draw_function(super::DrawLive2D::new()));
    }
    ...
    fn extract(
        ...
        Self::extract(self, world, transforms, surface_size);
    }

    fn collect_views(&self, views: &mut Vec<SceneView>) {
        Self::collect_views(self, views);
    }

    fn prepare(&mut self, gpu: &mut GpuContext, views: &[SceneView]) {
        Self::prepare(self, gpu, views);
    }
    ...
    fn append_phase_items(
        ...
        let Some(phase_view) = self.phase_views.get(view_index) else {
            return;
        };
    ...
    fn insert_frame_payloads<'a>(&'a self, frame: &mut PreparedFrame<'a>) {
        Self::insert_frame_payloads(self, frame);
    }
    ...
    fn insert_view_payloads<'a>(
        ...
        Self::insert_view_payloads(self, view_index, view, prepared_view);
    }
}
