...
- The high-level public path is:
  ...
  - `TilemapRenderer` — ECS-facing component in `component.rs`.
...
- `tiled/mod.rs` — Tiled facade and public imported data types.
- `tiled/import.rs` — file and asset loading entry points.
- `tiled/parse.rs` — format parsing and map instantiation.
...
- `component.rs` — ECS-facing tilemap authoring types such as `TilemapRenderer` and `TilesetGrid`.
