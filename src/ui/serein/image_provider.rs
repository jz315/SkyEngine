...
use serein::expert::{UiDrawCommand, UiDrawList};
use serein::{ImageRef, ImageRefKind};
use serein_wgpu::{GpuImage, ImageState, Resources};
...
#[derive(Default)]
pub(crate) struct SkySereinImageStore {
...
pub(crate) struct SkySereinImagePrepareStatus {
    pub(crate) pending: bool,
    pub(crate) ready_changed: bool,
}

impl SkySereinImageStore {
    ...
    ) -> SkySereinImagePrepareStatus {
        self.ready.clear();
        self.pending_frame.clear();
        let mut status = SkySereinImagePrepareStatus::default();
    ...
    pub(crate) fn provider(&self) -> SkySereinResources<'_> {
        SkySereinResources { store: self }
    }
    ...
    fn poll_remote_images(&mut self, asset_server: &Assets) {
        ...
        for (key, receiver) in &self.pending_remote {
            match receiver.try_recv() {
                ...
                Ok(Err(error)) => {
                    eprintln!(
                        "[SkyEngine] serein image load failed for {}: {error}",
                        key.source()
                    );
                ...
                Err(TryRecvError::Disconnected) => {
                    eprintln!(
                        "[SkyEngine] serein image load worker disconnected for {}",
                        key.source()
                    );
                    self.failed.insert(key.clone(), Instant::now());
                    completed.push(key.clone());
    ...
    fn resolve_handle(
        ...
        match load_texture_handle(asset_server, key) {
            ...
            Err(error) => {
                eprintln!(
                    "[SkyEngine] serein image asset lookup failed for {}: {error}",
                    key.source()
                );
...
pub(crate) struct SkySereinResources<'a> {
    store: &'a SkySereinImageStore,
}

impl Resources for SkySereinResources<'_> {
...
fn load_remote_texture_asset(key: &ImageRef) -> Result<TextureAsset, String> {
    ...
    {
        let _ = key;
        Err("remote serein image loading requires the `ui-serein-net` feature".to_string())
    }
...
mod tests {
    ...
    fn ready_image_revision_reports_only_new_texture_versions() {
        let mut store = SkySereinImageStore::default();
