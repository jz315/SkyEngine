...
use serein::{FontRef, Runtime};
use serein_wgpu::WgpuRenderer;

use crate::asset::{AssetId, Assets, FontAsset, Handle};

#[derive(Default)]
pub(crate) struct SkySereinFontStore {
...
pub(crate) struct SkySereinFontPrepareStatus {
    pub(crate) pending: bool,
    pub(crate) ready_changed: bool,
}

impl SkySereinFontStore {
    ...
    ) -> SkySereinFontPrepareStatus {
        self.pending_frame.clear();
        let mut status = SkySereinFontPrepareStatus::default();
    ...
    fn resolve_handle(
        ...
        match load_font_handle(asset_server, key) {
            ...
            Err(error) => {
                eprintln!("[SkyEngine] serein font asset lookup failed for {source}: {error}");
...
mod tests {
    ...
    fn ready_font_revision_reports_only_new_asset_versions() {
        let mut store = SkySereinFontStore::default();
