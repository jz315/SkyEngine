...
fn layout_element(element: &mut Element, measured: &MeasuredNode, x: f32, y: f32) {
    ...
    match element.kind {
        ElementKind::Row => layout_row(element, &layout_measured),
        ElementKind::Column => layout_column(element, &layout_measured),
        ElementKind::Stack => layout_stack(element, &layout_measured),
...
fn layout_row(element: &mut Element, measured: &MeasuredNode) {
        ...
        .zip(child_widths)
    {
        ...
        let child_y = content.y
            ...
            + child.margin.top;
        layout_element(child, &adjusted, child_x, child_y);
