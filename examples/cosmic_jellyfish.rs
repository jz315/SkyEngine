...
fn main() {
    ...
    for _ in 0..NUM_JELLYFISH {
        ...
        world.spawn((
            Position {
                x: rng.range(-640.0, 640.0),
                y: rng.range(-360.0, 360.0),
            },
            Drift {
    ...
    let stars: Vec<Star> = (0..NUM_STARS)
        .map(|_| Star {
            x: rng.range(-680.0, 680.0),
            y: rng.range(-400.0, 400.0),
    ...
    let graph: Rc<RefCell<_>> = Rc::new(RefCell::new(graph));
    ...
    App::run_with_lifecycle(
        ...
        // ── setup ───────────────────────────────────────────────────────
        |_world, _gpu| {
            eprintln!("[cosmic_jellyfish] Move mouse to steer the spotlight. Press Escape to exit.");
        },
        // ── frame ───────────────────────────────────────────────────────
        move |ctx| {
            ...
            *frame_time.borrow_mut() += dt;
            ...
            let half_w = w as f32 * 0.52;
            let half_h = h as f32 * 0.52;
            {
                ...
                q.for_each(&world, |(pos, drift, jelly)| {
                    ...
                    // Wrap around
                    if pos.y < -half_h { pos.y = half_h; pos.x = (pos.x + 200.0) % (half_w * 2.0) - half_w; }
            ...
            // Stars background
            {
                ...
                for star in stars.iter() {
                    ...
                    rs.scene_batch.draw(
                        Sprite::new(star.x, star.y, star.size, star.size)
                            .color(Color::new(twinkle, twinkle * 0.95, twinkle * 1.05, 0.9)),
                    );
                }
            }
            ...
            for jv in &visuals {
                ...
                // Bell (body)
                rs.scene_batch.draw(
                    Sprite::new(jv.x, jv.y, body_size, body_size * 0.7)
                        .color(Color::new(body_color.r * 1.3, body_color.g * 1.3, body_color.b * 1.3, 0.85)),
                );
                ...
                rs.scene_batch.draw(
                    Sprite::new(jv.x, jv.y - body_size * 0.05, glow_size, glow_size * 0.5)
                        .color(Color::new(
                            body_color.r * 2.0,
                            body_color.g * 2.0,
                            body_color.b * 2.0,
                ...
                for seg in 0..TENTACLE_SEGMENTS {
                    ...
                    rs.scene_batch.draw(
                        Sprite::new(tx, ty, seg_size, seg_size)
                            .color(Color::new(tentacle_color.r, tentacle_color.g, tentacle_color.b, alpha)),
                    );
                }
