//! # Boids — Bevy ECS version
//!
//! Same boids simulation, but using bevy_ecs for the ECS layer.
//! Compare FPS with the Sky Engine and hecs versions.
//!
//! ```
//! cargo run --example boids_bevy --features demo --release
//! ```
...
fn main() {
    ...
    while window.is_open() && !window.is_key_down(Key::Escape) {
        ...
        {
            let mut q = world.query::<(Entity, &mut Attractor)>();
            for (e, mut attr) in q.iter_mut(&mut world) {
                attr.life -= dt;
                if attr.life <= 0.0 {
                    dead.push(e);
                }
        ...
        {
            let mut idx = 0usize;
            let mut q = world.query::<(&Pos, &mut Vel, &Boid)>();
            for (pos, mut vel, _) in q.iter_mut(&mut world) {
                ...
                let pdx = pos.x - mouse_pos.0;
                let pdy = pos.y - mouse_pos.1;
                ...
                if pos.x > W as f32 - margin {
                    vel.x -= turn * dt;
                }
                if pos.y < margin {
                    vel.y += turn * dt;
                }
                if pos.y > H as f32 - margin {
                    vel.y -= turn * dt;
                }

                let speed = (vel.x * vel.x + vel.y * vel.y).sqrt();
                let target_max = if panic_mode {
                    MAX_SPEED * 1.5
                } else {
                    MAX_SPEED
                };
        ...
        // Move
        {
            let mut q = world.query::<(&mut Pos, &Vel, &Boid)>();
            for (mut pos, vel, _) in q.iter_mut(&mut world) {
                pos.x += vel.x * dt;
                pos.y += vel.y * dt;
                if !(pos.x >= 0.0) {
                    pos.x = 0.0;
                }
                if !(pos.x <= W as f32 - 1.0) {
                    pos.x = W as f32 - 1.0;
                }
                if !(pos.y >= 0.0) {
                    pos.y = 0.0;
                }
                if !(pos.y <= H as f32 - 1.0) {
                    pos.y = H as f32 - 1.0;
                }
            }
        }
