...
use sky_engine::asset::AssetServer;
...
impl AppState for LastLightGuild {
    ...
    fn update(&mut self, ctx: &mut FrameContext<'_>) {
        ...
        if ctx.input.key_pressed(KeyCode::KeyR) {
            let asset_server = ctx.world.get_resource::<AssetServer>().cloned();
...
fn main() {
    App::new(
        AppConfig::new("Last Light Guild", layout::WINDOW_W, layout::WINDOW_H)
            .with_vsync(true)
