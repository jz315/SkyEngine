...
//!
//! ```bash
//! cargo run --example ui_serein_eui_gallery --features ui-serein --release
//! ```

use std::sync::mpsc::{self, Receiver, TryRecvError};
#[cfg(feature = "ui-serein-net")]
...
use sky_engine::ui::serein::widgets::theme::{self, PageVisualTokens, ThemeColorTokens};
use sky_engine::ui::serein::{
    open_window, Align, AnimProperty, Color, Ease, HorizontalAlign, SereinWindowConfig, Signal,
    State, Transition, Ui,
};
...
impl Default for EuiSereinGallery {
    fn default() -> Self {
        Self {
...
impl Default for GalleryState {
    fn default() -> Self {
        Self {
            selected_page: env_i32("SKY_SEREIN_GALLERY_PAGE").unwrap_or(0).clamp(0, 5),
            option_dense: false,
            option_glass: env_flag("SKY_SEREIN_GALLERY_GLASS"),
            option_motion: true,
            ...
            animation_glowing: env_flag("SKY_SEREIN_GALLERY_ANIMATION_GLOWING"),
            sample_checked: true,
            sample_switch: true,
            sample_radio_a: true,
            ...
            sample_dialog_open: env_flag("SKY_SEREIN_GALLERY_DIALOG_OPEN"),
            sample_toast_visible: env_flag("SKY_SEREIN_GALLERY_TOAST_VISIBLE"),
            sample_context_menu_open: env_flag("SKY_SEREIN_GALLERY_CONTEXT_OPEN"),
            sample_context_menu_pos: [820.0, 390.0],
            sample_date_open: env_flag("SKY_SEREIN_GALLERY_DATE_OPEN"),
            sample_year: 2026,
            sample_month: 4,
            sample_day: 28,
            sample_time_open: env_flag("SKY_SEREIN_GALLERY_TIME_OPEN"),
            sample_hour: 9,
            sample_minute: 30,
            sample_color_open: env_flag("SKY_SEREIN_GALLERY_COLOR_OPEN"),
...
impl AppState for EuiSereinGallery {
    ...
    fn update(&mut self, ctx: &mut FrameContext<'_>) {
        ...
        if self.state.read(|state| state.sample_inspector_open) {
            ...
            open_window(
                ctx,
                SereinWindowConfig::new("Inspector", 640, 420)
                    .page_id("inspector")
                    .modal(true)
...
fn draw_inspector_window_content(
    ui: &mut Ui,
    screen: sky_engine::ui::serein::Screen,
    tokens: ThemeColorTokens,
) {
    let page = theme::page_visuals(tokens);
...
fn bing_api_text() -> String {
    #[cfg(not(feature = "ui-serein-net"))]
    {
        "Network text support requires the `ui-serein-net` feature.".to_string()
    }
...
fn main() {
    ...
    App::new(world).run(EuiSereinGallery::default());
}
