...
fn main() {
    ...
    world
        .install(
            WindowPlugin::new("Serein Control Center", 1440, 920)
                .with_vsync(true)
                .with_resizable(true),
        )
        .unwrap();
    ...
    App::new(world).run(app::SereinControlCenter::default());
}

#[cfg(test)]
mod tests {
    use sky_engine::ui::serein::expert::UiDrawCommand;
    use sky_engine::ui::serein::{Color, Frame, FrameInput, PointerEvent, Runtime, Screen};

    use super::model::{AppModel, Page};
    use super::theme;
    use super::view::{self, RuntimeInfo};

    fn compose_control_center(runtime: &mut Runtime, state: &sky_engine::ui::serein::State<AppModel>) {
    ...
    fn compose_control_center_incremental(
        runtime: &mut Runtime,
        state: &sky_engine::ui::serein::State<AppModel>,
    ) {
        let snapshot = state.read(Clone::clone);
        let runtime_info = RuntimeInfo {
            uptime_seconds: 0.0,
    ...
    fn nav_color(frame: &Frame, index: usize) -> Color {
        let id = format!("serein.control-center.nav.{index}.bg");
        frame
            .draw_list()
            .commands()
            .iter()
    ...
    fn sidebar_nav_buttons_switch_pages_from_their_visual_bounds() {
        let state = sky_engine::ui::serein::State::new(AppModel::default());
    ...
    fn sidebar_nav_buttons_switch_pages_through_incremental_compose() {
        ...
        assert_eq!(
            ...
            sky_engine::ui::serein::expert::LayoutMode::Full(
                sky_engine::ui::serein::expert::FullLayoutReason::StructureChanged {
                    ids: vec!["serein.control-center.workspace".into()]
                }
            )
        );
        assert_eq!(
            ...
            vec![
                "serein.control-center.nav".to_string(),
                "serein.control-center.workspace".to_string(),
            ]
        );
