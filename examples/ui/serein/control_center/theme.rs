use sky_engine::ui::serein::widgets::theme as serein_theme;
use sky_engine::ui::serein::widgets::theme::{PageVisualTokens, ThemeColorTokens};
...
pub fn resolve(mode: ThemeMode) -> AppTheme {
    ...
    let _page: PageVisualTokens = serein_theme::page_visuals(tokens);
    ...
    AppTheme {
        ...
        text_soft: serein_theme::with_opacity(tokens.text, 0.78),
        text_muted: serein_theme::with_opacity(tokens.text, 0.58),
...
pub fn alpha(color: Color, value: f32) -> Color {
    serein_theme::with_alpha(color, value)
}

pub fn mix(from: Color, to: Color, amount: f32) -> Color {
    serein_theme::mix_color(from, to, amount)
}
