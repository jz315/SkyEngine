...
//!
//! ```bash
//! cargo run --example ui_serein_scroll_xy --features ui-serein --release
//! ```
...
impl Default for SereinScrollXYDemo {
    fn default() -> Self {
        Self {
...
impl AppState for SereinScrollXYDemo {
...
fn draw_canvas(ui: &mut sky_engine::ui::serein::Ui) {
    for y in 0..5 {
        for x in 0..7 {
...
fn main() {
    ...
    world
        .install(
            WindowPlugin::new("Serein Scroll XY Demo", WINDOW_W, WINDOW_H)
                .with_vsync(false)
                .with_resizable(true),
        )
        .unwrap();
    ...
    App::new(world).run(SereinScrollXYDemo::default());
}
