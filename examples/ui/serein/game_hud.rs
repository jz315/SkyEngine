//! Game HUD slice built with `ui-serein`.
//!
//! The layout is an original fantasy RPG mock UI intended to pressure-test
//! Serein UI for game HUD work: layered HUD chrome, party selection, command
//! buttons, status bars, and an inventory sheet.
//!
//! ```bash
//! cargo run --example ui_serein_game_hud --features ui-serein --release
//! ```
...
impl Default for SereinGameHudDemo {
    fn default() -> Self {
        Self {
            time: 0.0,
...
impl Default for GameUiState {
    fn default() -> Self {
        Self {
            page: env_i32("SKY_SEREIN_GAME_PAGE").unwrap_or(0).clamp(0, 1),
            active_party: 0,
            combo: 0,
            skill_pulse: false,
        }
...
impl AppState for SereinGameHudDemo {
    ...
    fn update(&mut self, ctx: &mut FrameContext<'_>) {
        ...
        ctx.set_title(if title_page == 0 {
            "SkyEngine Serein Game HUD"
        } else {
            "SkyEngine Serein Game HUD - Inventory"
        });
        ctx.render();
    ...
    ui.stack("inventory")
        ...
        .content(|ui| {
            ...
            ui.text("inventory.subtitle")
                .position(32.0, 59.0)
                .size(360.0, 22.0)
                .text("Prototype inventory surface composed in Serein UI")
                .font_size(14.0)
                .line_height(22.0)
...
fn main() {
    ...
    world
        .install(
            WindowPlugin::new("SkyEngine Serein Game HUD", WINDOW_W, WINDOW_H)
                .with_vsync(false)
                .with_resizable(true),
        )
        .unwrap();
    ...
    App::new(world).run(SereinGameHudDemo::default());
}
