//! Visual stress lab for the Serein UI backend.
//!
//! ```bash
//! cargo run --example ui_serein_stress_lab --features ui-serein --release
//! ```
...
#[cfg(feature = "egui")]
use sky_engine::ui::serein::LayoutRect;
use sky_engine::ui::serein::{
    Align, AnimProperty, Color, Ease, FrameInput, HorizontalAlign, SereinUiBackend, PointerEvent,
...
struct SereinUiStressLab {
    ...
    layout_dumped: bool,
    draw_dumped: bool,
    agent_debug: Option<sky_engine::ui::serein::expert::AgentDebugService>,
    #[cfg(feature = "egui")]
    debug: SereinEguiDebug,
}
...
impl Default for SereinUiStressLab {
    fn default() -> Self {
        Self {
            time: 0.0,
            title_timer: 1.0,
            ...
            layout_dumped: false,
            draw_dumped: false,
            agent_debug: sky_engine::ui::serein::expert::AgentDebugService::from_env("stress_lab"),
            #[cfg(feature = "egui")]
            debug: SereinEguiDebug::default(),
        }
    }
}
...
impl Default for LabState {
    fn default() -> Self {
        Self {
            ...
            dropdown_open: false,
            dropdown_selected: 1,
            dialog_open: env_flag("SKY_SEREIN_LAB_DIALOG_OPEN"),
            toast_visible: env_flag("SKY_SEREIN_LAB_TOAST_VISIBLE"),
            context_menu_open: env_flag("SKY_SEREIN_LAB_CONTEXT_OPEN"),
...
impl AppState for SereinUiStressLab {
    ...
    fn update(&mut self, ctx: &mut FrameContext<'_>) {
        ...
        if self.title_timer >= 0.25 || perf.trigger_active {
            ctx.set_title(&format!(
                "SkyEngine - Serein UI Stress Lab | mode {mode} | clicks {clicks} | {:.0} FPS | {:.1} ms",
...
impl SereinUiStressLab {
    fn draw_ui(&mut self, ctx: &mut FrameContext<'_>, perf: &PerfSnapshot) {
        ...
        let ui_state = state_store.clone();
        ...
        if env_flag("SKY_SEREIN_LAB_DUMP_LAYOUT") && !self.layout_dumped {
            let mut ui = ctx.ui();
        ...
        if env_flag("SKY_SEREIN_LAB_DUMP_DRAW") && !self.draw_dumped {
            let mut ui = ctx.ui();
            ui.with_backend_mut::<SereinUiBackend, _>(|backend| {
        ...
        if env_flag("SKY_SEREIN_LAB_TRACE_CHART") {
    ...
    fn update_agent_debug(&mut self, ctx: &mut FrameContext<'_>) {
        ...
        let context = sky_engine::ui::serein::expert::AgentDebugContext::new()
        ...
        for effect in effects {
            match effect {
                sky_engine::ui::serein::expert::AgentDebugEffect::Screenshot { path } => {
    ...
    fn draw_egui_debug(&mut self, ctx: &mut FrameContext<'_>) {
        ...
        let mut data = None;
        {
            let mut ui = ctx.ui();
            ui.with_backend_mut::<SereinUiBackend, _>(|backend| {
                data = Some(SereinDebugPanelData::collect(
                    backend.runtime(),
                    lab_state,
                    &element_filter,
                    &draw_filter,
    ...
    fn maybe_auto_click_chart(&mut self, ctx: &mut FrameContext<'_>) {
        let Some(click_frame) = env_u32("SKY_SEREIN_LAB_AUTO_CLICK_CHART_FRAME") else {
            return;
        };
        ...
        ui.with_backend_mut::<SereinUiBackend, _>(|backend| {
            ...
            if let Some(frame) = target_frame {
                ...
                eprintln!(
                    "[serein auto click] target=signals.tabs.hit.1 frame=({:.2},{:.2},{:.2},{:.2}) point=({:.2},{:.2}) input_changed={} needs_compose={}",
                    frame.x,
                    frame.y,
                    frame.width,
                    frame.height,
        ...
        if clicked {
            ...
            eprintln!("[serein auto click] missing target=signals.tabs.hit.1");
        }
    }
}
...
#[cfg(feature = "egui")]
impl Default for SereinEguiDebug {
    fn default() -> Self {
        Self {
            visible: !env_flag("SKY_SEREIN_LAB_EGUI_DEBUG_OFF"),
            element_filter: env_string("SKY_SEREIN_LAB_EGUI_FILTER")
                .unwrap_or_else(|| "controls.preset.dropdown".to_string()),
            draw_filter: env_string("SKY_SEREIN_LAB_EGUI_DRAW_FILTER")
                .unwrap_or_else(|| "controls.preset.dropdown".to_string()),
            max_rows: 80,
            show_elements: true,
            show_draw: true,
...
#[cfg(feature = "egui")]
impl SereinEguiDebug {
    fn show(&mut self, root_ui: &mut sky_engine::app::egui::Ui, data: &SereinDebugPanelData) {
        ...
        if !self.visible {
            egui::Area::new(egui::Id::new("serein_debug_toggle"))
                .order(egui::Order::Foreground)
                .fixed_pos([12.0, 12.0])
                .show(root_ui.ctx(), |ui| {
                    if ui.button("Serein Debug").clicked() {
                        self.visible = true;
                    }
                });
            return;
        ...
        egui::Window::new("Serein Debug")
            ...
            .show(root_ui.ctx(), |ui| {
                ...
                egui::CollapsingHeader::new("State")
                    .default_open(true)
                    .show(ui, |ui| {
                        egui::Grid::new("serein_debug_state")
                            .num_columns(2)
                            .striped(true)
                            .show(ui, |ui| {
                ...
                egui::CollapsingHeader::new("Input")
                    .default_open(false)
                    .show(ui, |ui| {
                        egui::Grid::new("serein_debug_input")
                            .num_columns(2)
                            .striped(true)
                            .show(ui, |ui| {
...
#[cfg(feature = "egui")]
impl SereinDebugPanelData {
    fn collect(
        runtime: &sky_engine::ui::serein::Runtime,
        state: LabDebugState,
        element_filter: &str,
        draw_filter: &str,
        max_rows: usize,
...
#[cfg(feature = "egui")]
impl LayerSummary {
    fn from_record(record: &sky_engine::ui::serein::expert::LayerDebugRecord) -> Self {
        Self {
            id: record.id.clone(),
            open: record.open,
...
fn collect_element_summary(
    element: &sky_engine::ui::serein::expert::Element,
    parent: Option<&str>,
    filter: &str,
    max_rows: usize,
    total: &mut usize,
...
fn draw_summary(
    index: usize,
    command: &sky_engine::ui::serein::expert::UiDrawCommand,
    filter: &str,
) -> Option<DrawSummary> {
    use sky_engine::ui::serein::expert::UiDrawCommand;
...
#[cfg(feature = "egui")]
fn show_runtime_summary(ui: &mut sky_engine::app::egui::Ui, data: &SereinDebugPanelData) {
    sky_engine::app::egui::Grid::new("serein_debug_summary")
        .num_columns(2)
        .striped(true)
        .show(ui, |ui| {
...
#[cfg(feature = "egui")]
fn show_layers(ui: &mut sky_engine::app::egui::Ui, data: &SereinDebugPanelData) {
    sky_engine::app::egui::Grid::new("serein_debug_layers")
        .num_columns(8)
        .striped(true)
        .show(ui, |ui| {
            header(ui, "open");
...
#[cfg(feature = "egui")]
fn show_elements(ui: &mut sky_engine::app::egui::Ui, data: &SereinDebugPanelData) {
    sky_engine::app::egui::ScrollArea::vertical()
        .max_height(260.0)
        .show(ui, |ui| {
            sky_engine::app::egui::Grid::new("serein_debug_elements")
                .num_columns(9)
                .striped(true)
                .show(ui, |ui| {
                    header(ui, "z");
...
#[cfg(feature = "egui")]
fn show_draw(ui: &mut sky_engine::app::egui::Ui, data: &SereinDebugPanelData) {
    sky_engine::app::egui::ScrollArea::vertical()
        .max_height(260.0)
        .show(ui, |ui| {
            sky_engine::app::egui::Grid::new("serein_debug_draw")
                .num_columns(5)
                .striped(true)
                .show(ui, |ui| {
                    header(ui, "#");
...
impl Default for ScreenshotProbe {
    fn default() -> Self {
        Self {
            ...
            frame: env_u32("SKY_SEREIN_SCREENSHOT_FRAME").unwrap_or(45),
            second_path: std::env::var("SKY_SEREIN_SCREENSHOT_PATH_2")
                .ok()
                .filter(|value| !value.trim().is_empty()),
            second_frame: env_u32("SKY_SEREIN_SCREENSHOT_FRAME_2").unwrap_or(u32::MAX),
            frame_count: 0,
            taken: false,
            second_taken: false,
...
fn maybe_log_stutter(perf: &PerfSnapshot, compose_ms: f32, overlay_ms: f32) {
    let Some(threshold_ms) = env_f32("SKY_SEREIN_LAB_STUTTER_LOG_MS") else {
        return;
    };
    ...
    eprintln!(
        "[serein stutter] frame={} frame_ms={:.3} avg_ms={:.3} compose={:.3} overlay={:.3} rest={:.3} lifetime_worst={:.3} trigger={} trigger_age_ms={:.1} threshold={:.3}",
        perf.frame,
        perf.frame_ms,
        perf.avg_ms,
        compose_ms,
...
fn dump_runtime_layout(label: &str, runtime: &sky_engine::ui::serein::Runtime) {
    let snapshot = runtime.diagnostics().current_snapshot();
    eprintln!(
        "[serein layout dump] {label}: screen={:?} layout={:?} dirty={:?} live={:?} built={} reused={} active_animations={} needs_render={} needs_compose={} full_redraw={}",
        snapshot.screen,
    ...
    eprintln!("[serein layout dump end] {label}");
}

fn dump_runtime_draw(label: &str, runtime: &sky_engine::ui::serein::Runtime) {
    let trace = runtime.diagnostics().draw_trace();
    let filter = env_string("SKY_SEREIN_LAB_DUMP_DRAW_FILTER");
    eprintln!(
        "[serein draw dump] {label}: screen={:?} commands={} primitives={} push_clip={} pop_clip={} max_clip_depth={} unbalanced_pops={} remaining_clip_depth={} filter={:?}",
        trace.screen,
        trace.command_count,
    ...
    if env_flag("SKY_SEREIN_LAB_DUMP_DRAW_JSON") && filter.is_none() {
    ...
    eprintln!("[serein draw dump end] {label}");
}

fn dump_element_layout(element: &sky_engine::ui::serein::expert::Element, depth: usize) {
...
fn trace_chart_runtime(label: &str, tab: i32, time: f32, runtime: &sky_engine::ui::serein::Runtime) {
    let snapshot = runtime.diagnostics().current_snapshot();
    let target = runtime
        .diagnostics()
    ...
    let draw = runtime.draw_list().commands().iter().find_map(|command| {
        if let sky_engine::ui::serein::expert::UiDrawCommand::Rect(rect) = command {
            rect.id
    ...
    eprintln!(
        "[serein chart trace] {label} app_time={time:.3} tab={tab} runtime_frame={} layout={:?} dirty={:?} live={:?} target={target:?} draw={draw:?} active_animations={} needs_render={} full_redraw={}",
...
fn main() {
    ...
    world
        .install(
            WindowPlugin::new("SkyEngine - Serein UI Stress Lab", WINDOW_W, WINDOW_H)
                .with_vsync(false)
                .with_resizable(true),
        )
        .unwrap();
    ...
    App::new(world).run(SereinUiStressLab::default());
}

#[cfg(test)]
mod tests {
    use super::*;
    use sky_engine::ui::serein::{FrameInput, Runtime, Screen, UiTestDriver};
    ...
    fn frame_lab_driver_at(driver: &mut UiTestDriver, state: &State<LabState>, time: f32) {
        let perf = perf_snapshot();
        let ui_state = state.clone();
        driver.frame_with_delta(time, move |ui, screen| {
            draw_lab(
                ui,
                screen.width,
                screen.height,
    ...
    fn dirty_contains(dirty: &[(String, sky_engine::ui::serein::DirtyFlags)], id: &str) -> bool {
    ...
    fn signals_tab_click_uses_driver_and_rebuilds_precise_signal_owners() {
        ...
        let trace = driver
            .click_at(
                ...
                sky_engine::ui::serein::TargetPoint::Center,
            )
    ...
    fn stress_lab_control_dropdown_stays_open_after_app_style_pointer_frame() {
        let state = State::new(LabState::default());
        let mut runtime = Runtime::new("serein");
    ...
    fn rect_draw(runtime: &Runtime, id: &str) -> Option<sky_engine::ui::serein::expert::UiRectDraw> {
    ...
    fn text_draw_color(runtime: &Runtime, id: &str) -> Option<Color> {
        runtime.draw_list().commands().iter().find_map(|command| {
            if let sky_engine::ui::serein::expert::UiDrawCommand::Text(text) = command {
