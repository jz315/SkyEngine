//! Small `ui-serein` layout primitive demo.
//!
...
//!
//! ```bash
//! cargo run --example ui_serein_layout_primitives --features ui-serein --release
//! ```
...
use sky_engine::ui::serein::{Align, Color, Size};

const WINDOW_W: u32 = 1100;
const WINDOW_H: u32 = 760;

#[derive(Default)]
struct SereinLayoutPrimitives {
    screenshot: ScreenshotProbe,
}

impl AppState for SereinLayoutPrimitives {
...
fn header(ui: &mut sky_engine::ui::serein::Ui) {
    ui.row("header")
        .size(Size::fill(), 78.0)
        .gap(18.0)
...
fn toolbar(ui: &mut sky_engine::ui::serein::Ui) {
    ui.row("toolbar")
        .size(Size::fill(), 52.0)
        .gap(12.0)
        .align_items(Align::Center)
...
fn card_grid(ui: &mut sky_engine::ui::serein::Ui) {
    ui.row("grid")
...
fn footer(ui: &mut sky_engine::ui::serein::Ui) {
    ui.row("footer")
        .size(Size::fill(), 42.0)
        .gap(10.0)
        .align_items(Align::Center)
...
fn main() {
    ...
    world
        .install(
            WindowPlugin::new("Serein Layout Primitives", WINDOW_W, WINDOW_H)
                .with_vsync(false)
                .with_resizable(true),
        )
        .unwrap();
    ...
    App::new(world).run(SereinLayoutPrimitives::default());
}
