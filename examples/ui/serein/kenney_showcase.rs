//! Kenney UI pack showcase built with `ui-serein`.
//!
//! ```bash
//! cargo run --example ui_serein_kenney_showcase --features ui-serein --release
//! ```
...
use sky_engine::ui::serein::{
    ButtonSkin, CheckboxSkin, Color, EdgeInsets, FontRef, HorizontalAlign, ImageRef, SereinSkin,
...
struct KenneySereinUiShowcase {
    time: f32,
    state: State<DemoState>,
    screenshot: ScreenshotProbe,
}

impl Default for KenneySereinUiShowcase {
    fn default() -> Self {
        Self {
            time: 0.0,
...
fn initial_page_from_env() -> DemoPage {
    let Ok(value) = std::env::var("SKY_SEREIN_SHOWCASE_PAGE") else {
...
fn kenney_skin() -> SereinSkin {
    SereinSkin::new("kenney")
...
impl AppState for KenneySereinUiShowcase {
    ...
    fn update(&mut self, ctx: &mut FrameContext<'_>) {
        ...
        ctx.set_title(match snapshot.page {
            DemoPage::Title => "SkyEngine - Kenney Serein UI",
            DemoPage::Hud => "SkyEngine - Kenney Serein UI HUD",
            DemoPage::Loadout => "SkyEngine - Kenney Serein UI Loadout",
            DemoPage::Settings => "SkyEngine - Kenney Serein UI Settings",
        });
        ctx.render();
...
fn draw_loadout(
    ...
    ui.text("loadout.detail.copy")
        ...
        .size(118.0, 70.0)
        .text("Serein keeps the UI state local and lets the renderer resolve image assets.")
        .font_size(13.0)
        .line_height(18.0)
        .wrap(true)
...
fn main() {
    ...
    world
        .install(
            WindowPlugin::new("SkyEngine - Kenney Serein UI", WINDOW_W, WINDOW_H)
                .with_vsync(false)
                .with_resizable(true),
        )
        .unwrap();
    ...
    sky_engine::ui::serein::register_skin(&mut world, kenney_skin());
    world
    ...
    App::new(world).run(KenneySereinUiShowcase::default());
}
