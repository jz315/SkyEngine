//! Calculator demo built with `ui-serein`.
//!
//! ```bash
//! cargo run --example ui_serein_calculator --features ui-serein --release
//! ```
...
use sky_engine::ui::serein::{Align, Color, HorizontalAlign, Size, State, Ui};
...
impl Default for SereinCalculatorDemo {
    fn default() -> Self {
        Self {
...
impl AppState for SereinCalculatorDemo {
    fn setup(&mut self, ctx: &mut SetupContext<'_>) {
        ctx.world.insert_resource(RenderSettings {
            clear_color: sky_engine::render::Color::new(0.070, 0.082, 0.096, 1.0),
            ..Default::default()
        });
        ctx.world.spawn((
            Transform::default(),
...
fn main() {
    ...
    world
        .install(
            WindowPlugin::new("Serein Calculator", WINDOW_W, WINDOW_H)
                .with_vsync(false)
                .with_resizable(true),
        )
        .unwrap();
    ...
    App::new(world).run(SereinCalculatorDemo::default());
}
