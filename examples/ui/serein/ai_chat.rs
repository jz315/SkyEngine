//! AI chat application mockup built with `ui-serein`.
//!
//! ```bash
//! cargo run --example ui_serein_ai_chat --features ui-serein --release
//! ```
...
use sky_engine::ui::serein::widgets::theme;
use sky_engine::ui::serein::{Align, Color, HorizontalAlign, Size, State, Ui, VerticalAlign};
...
struct SereinAiChatDemo {
    state: State<ChatState>,
    screenshot: ScreenshotProbe,
}

impl Default for SereinAiChatDemo {
    fn default() -> Self {
        Self {
...
impl AppState for SereinAiChatDemo {
...
fn seed_messages(index: usize) -> Vec<ChatMessage> {
    match index {
        ...
        3 => vec![
            ChatMessage::assistant(
                "Yesterday",
                "The release note can stay small: new scroll_y, popover, and rounded_clip helpers for Serein UI.",
            ),
...
fn main() {
    ...
    world
        .install(
            WindowPlugin::new("Sky Chat - Serein AI", WINDOW_W, WINDOW_H)
                .with_vsync(false)
                .with_resizable(true),
        )
        .unwrap();
    ...
    App::new(world).run(SereinAiChatDemo::default());
}
